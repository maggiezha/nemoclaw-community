...
# Build a deployment-specific NemoClaw/Hermes image and push it to a registry that
# the remote OpenShell Kubernetes gateway can pull. No inference API key enters the image.
# Mirrors ../openclaw/build-sandbox-image.sh; see agents/hermes/Dockerfile upstream.
...
[[ -n "${SANDBOX_IMAGE}" ]] \
  || fail "set HERMES_SANDBOX_IMAGE to a registry image with a non-latest tag"
[[ "${SANDBOX_IMAGE}" =~ ^[A-Za-z0-9][A-Za-z0-9._:/@-]+$ ]] \
  || fail "HERMES_SANDBOX_IMAGE contains unsupported characters"
[[ "${SANDBOX_IMAGE}" == */* ]] \
  || fail "HERMES_SANDBOX_IMAGE must include a registry/repository path"
[[ "${SANDBOX_IMAGE}" != *@* && "${IMAGE_NAME}" == *:* ]] \
  || fail "HERMES_SANDBOX_IMAGE must be a tagged build target, not a digest"
[[ "${SANDBOX_IMAGE}" != *:latest ]] \
  || fail "HERMES_SANDBOX_IMAGE must not use the mutable latest tag"
...
HERMES_DOCKERFILE="${SOURCE_DIR}/agents/hermes/Dockerfile"
[[ -f "${HERMES_DOCKERFILE}" ]] \
  || fail "NemoClaw ${NEMOCLAW_VERSION} is missing agents/hermes/Dockerfile"
...
docker buildx build \
  ...
  --sbom=true \
  --tag "${SANDBOX_IMAGE}" \
  --file "${HERMES_DOCKERFILE}" \
  --build-arg "BASE_IMAGE=ghcr.io/nvidia/nemoclaw/hermes-sandbox-base:${NEMOCLAW_VERSION}" \
...
echo "Pushed deployment-specific NemoClaw/Hermes sandbox image: ${SANDBOX_IMAGE}"
